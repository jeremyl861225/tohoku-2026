...
| 里程 | **595 km／駕駛 11:06**（Google Maps 實測，2026-08-28） |
| 路線 | 仙台空港→仙台→松島→鳴子溫泉→銀山溫泉→山寺→米澤→會津若松→五色沼→遠刈田溫泉→藏王御釜→仙台空港 |
...
```js
{
  ...
  stay: "住宿", time: "15:00",
  lat: 38.5706, lng: 140.5305,   // 導航按鈕與座標顯示用
...
目前 **22 個景點、其中 11 個在地圖上有標記**。`id` 必須與 `makemap.py` 的
